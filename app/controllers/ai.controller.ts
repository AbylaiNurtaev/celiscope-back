...
router.post('/goal/from-template', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { template, deadline, maxItems, context } = req.body || {}
    const result = await aiService.generateGoalFromTemplate({ template, deadline, maxItems, context })
...
router.post('/goal/chat', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await aiService.chatAboutGoals({ question, context, history })
    res.status(200).json(result)
  } catch (err) {
    next(err)
  }
